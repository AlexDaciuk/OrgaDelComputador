				...
				while (has_codepoint(enc, &buf[b], *nbytes)) {
								...
								case UTF8:
												if ( (buf[b+0] & 0x80) == 0 ) {
																...
																*nbytes -= 3;
												} else if ( ((buf[b+0] & 0xF8) == 0xF0) && ((buf[b+1] & 0x80) == 0x80) && ((buf[b+2] & 0x80) == 0x80) && ((buf[b+3] & 0x80) == 0x80)) {
																cp |= (buf[b++] & 0x07) << 18;
																cp |= (buf[b++] & 0x7F) << 12;
																cp |= (buf[b++] & 0x7F) << 6;
																cp |= (buf[b++] & 0x7F);
...
int ucs4_to_dest(enum encoding enc, uint32_t *input, int npoints, uint8_t *outbuf) {
				...
				for (int i=0; i < npoints; i++) {
								...
								case UTF16LE:
												...
												break;
								case UTF8:
												if (((cp >> 8) & 0xFF) >= 0x01 && ((cp >> 8) & 0xFF) <= 0x10) {
																outbuf[b++] = ((cp >> 10) & 0x07) | 0xF0;
																outbuf[b++] = (((cp >> 4) & 0x30) | ((cp >> 20) & 0x0F)) | 0x80;
																outbuf[b++] = (((cp >> 14) & 0x3C) | ((cp >> 30) & 0x03)) | 0x80;
												...
												else if (((cp >> 24) & 0xFF) <= 0x7F && ((cp >> 16) & 0xFF) == 0x00) {
																outbuf[b++] = (cp >> 24) & 0x7F;
												}
